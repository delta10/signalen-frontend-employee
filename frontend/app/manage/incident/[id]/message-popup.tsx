...
import { 
  MapPin, Clock, User, Phone, Mail, MessageSquare, Edit, 
  ExternalLink, Calendar, FileText, Users, Building, 
  Activity, ChevronRight, ChevronDown, History, ArrowUpRight, 
  TriangleAlert, Minus, LucideIcon, MoreVertical, FilePlus, 
  Share2, BadgeAlert, CircleCheck, CirclePlay, RotateCcw, CircleX, Loader2, X
} from 'lucide-react';
...
// --- Algemene Link & Paginering Interfaces ---
...
const getPriorityText = (priority: Report['priority'] | string): string => {
  ...
  switch (priorityString?.toLowerCase()) {
    ...
    case 'normal': default: return 'Normaal';
  }
};
...
export default function MessageModalPopup({ initialReport, id, attachments: initialAttachments, history: initialHistory, context: initialContext, statusMessages: initialStatusMessages, users: initialUsers, relatedReporters: initialRelatedReporters, isIntercepted = false}: MessageModalPopupProps) {
  ...
  const defaultStatusOptions: StatusMessage[] = [
  ...
  const statesWithoutTextRequirement = ['m', 'i', 'a', 'b', 's'];
  ...
  return !statesWithoutTextRequirement.includes(matchedStatus.state);
};
  ...
  const priorityColors: Record<string, PriorityDetails> = {
    'low': { icon: Minus, color: 'bg-muted-foreground text-secondary', text: 'Laag', variant: 'laag' },
    'normal': { icon: Activity, color: 'text-foreground', text: 'Normaal', variant: 'normaal' },
    'high': { icon: TriangleAlert, color: 'bg-high text-5xl', text: 'Hoog', variant: 'hoog' },
  };
    ...
    try {
      ...
      if (status !== report.status.state_display) {
        ...
        if (matchedStatusMessage) {
          ...
          const requiresText = !statesWithoutTextRequirement.includes(matchedStatusMessage.state);
                              ...
                              return (
                                ...
                                > 
                                  <div className="flex items-center gap-2">
                                    {statusIcons[displayText] && 
                                      React.createElement(statusIcons[displayText], { 
                                        className: 'h-3.5 w-3.5'
                                      })
                                    }
